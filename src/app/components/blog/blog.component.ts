...
export class BlogComponent implements OnInit{
  ...
  guardar(): void {
    ...
    if(this.title !== "" && this.img !== "" && this.text !== "" && this.date !== "") {
  ...
  cargarNoticias(): void {
    ...
    this.arrNoticias.forEach((noticia) => {
      this.listaNoticias += `<div class="card"> <h3 class="card-title">${noticia.title}</h3> <img class="card-img" src = "${noticia.img}"/> <p class="card-text">${noticia.text}</p> <p class="card-date">${noticia.date}</p> </div>`;
    });
  }
